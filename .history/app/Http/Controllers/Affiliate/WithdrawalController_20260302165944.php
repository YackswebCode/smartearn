  ...
  public function index(Request $request)
{
    ...
    // Completed withdrawals (from transactions)
    $completedWithdrawals = Transaction::where('user_id', $user->id)
        ...
        ->map(function ($tx) use ($userCurrency, $toNGN) {
            ...
            return [
                ...
                'status' => ucfirst($tx->status), // completed, failed
            ];
        });

    // Merge and sort
    $allWithdrawals = $pendingWithdrawals->concat($completedWithdrawals)
        ->sortByDesc('date')
        ->values();
