...
<div class="container-fluid py-4">
    ...
    <div class="mb-4">
        ...
        <p class="text-muted">{{ $product->exists ? 'Update your product information' : 'Fill in the details to add a new product' }}</p>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (removeBtn) {
            removeBtn.addEventListener('click', function(e) {
                ...
                // If there's an existing image, we cannot delete it server-side yet; user must submit form with no new file to keep old.
                // Optionally, you could add a hidden field to mark deletion, but for simplicity we'll just clear.
            });
        }

        // If there's an existing image, the preview is shown; but we need to allow replacement.
        // The remove button just clears the file input and hides preview; the old image remains until form submission.
        // To delete old image, user must upload new one or remove and submit (but without a new file, old remains).
        // We could add a "remove permanently" checkbox, but for now this is fine.
    });
</script>
@endpush